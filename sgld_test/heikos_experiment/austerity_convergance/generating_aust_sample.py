...
from sgld_test.likelihoods import gen_X, log_probability, _log_lik
...
def vectorized_log_lik(X,theta):
     return _log_lik(theta[0],theta[1],X)
...
sample = austerity(vectorized_log_lik,log_density_prior, X,0.01,batch_size=50,chain_size=100, thinning=15, theta_t=np.random.randn(2))
