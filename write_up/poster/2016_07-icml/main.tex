...
\begin{align*}
MMD(p,q,G)^{2}  =\ev_{q}\langle\xi_{p}(Z),\xi_{p}(Z')\rangle_{\mathcal{F}^{d}}=\ev_{q}h_{p}(Z,Z'),
\end{align*}
\end{block}
...
\large
If the kernel $k$ is $C_0$-universal, $\ev_{{ \color{blue} q}} h_{{ \color{blue} q}}(Z,Z)<\infty$ and $\ev_{{ \color{blue} q}} (\log' \frac{{\color{red} p}(Z)}{{ \color{blue} q}(Z)})^{2}<\infty$
...
$p\neq q$, but $MMD({\color{red} p},{ \color{blue} q},G) =0$. If $MMD({\color{red} p},{ \color{blue} q},G) =0$ then, by Theorem 1 
$\ev_{q}\xi_{p}(Z)=0$. We substitute $\log p(Z)=\log q(Y)+[\log p(Z)-\log q(Y)]$,
\begin{align*}
 & \ev_{q}\xi_{p}(Z)\\
...
to the measure $q$.  $g$ is square integrable,
...
\]
Consequently  $\log\frac{p(y)}{q(y)}=C$, for some $C$, and so  $p(y)=e^{C}q(y)$. Since $p$ and $q$ both integrate
...
$\ev h(Z,Z)<\infty$. Under the null hypothesis $nV_n$ and $nB_n$ have the same limiting distribution (in a weak sense). Under the alternative hypothesis,
