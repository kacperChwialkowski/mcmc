...
\subsection{Student t vs Normal}
This sani

In this experiment we check that one-sample test can reject a hypothesis that a sample, drawn form a t-student distribution, comes form a normal distribution. For a student t distribution with a number of degrees of freedom $k$, we draw 500 samples and calculate the p-value -- this procedure is repeated 150 times so  150 p-values are calculated and a bar plot is created. Then we vary the number of degrees of freedom in the range $1,6,\cdots,71$ and bar plots for each of $k$ are calculated. 
